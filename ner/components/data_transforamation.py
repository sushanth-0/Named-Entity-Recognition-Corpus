...
class DataTransformation:
    ...
    def clean_and_convert_labels(self, df: DataFrame) -> DataFrame:
        try:
            # Remove the 'Sentence #' and 'POS' columns, and rename 'Sentence' to 'text' and 'Tag' to 'labels'
            df = df.drop(columns=['Sentence #', 'POS'])
            df = df.rename(columns={'Sentence': 'text', 'Tag': 'labels'})

            # Convert the string representation of lists in 'labels' to actual lists
            df['labels'] = df['labels'].apply(
                lambda x: ' '.join(ast.literal_eval(str(x))))
            return df
    ...
    def splitting_data(self, df: DataFrame) -> dict:
        logging.info(
            "Entered the splitting_data method of Data transformation class")
        try:
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifacts:
        ...
        try:
            ...
            df = self.clean_and_convert_labels(df)
