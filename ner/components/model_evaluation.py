...
class ModelEvaluation:
    ...
    def evaluate(self, model: object, df_test: DataFrame) -> float:
        try:
            logging.info(
                "Entered the evaluate method of Model evaluation class")
            ...
            logging.info(
                "Exited the evaluate method of Model evaluation class")
            return val_accuracy
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        try:
            ...
            trained_model_accuracy = self.evaluate(
                model=model, df_test=df_test)
            logging.info(
                f"The accuracy on test dataset is - {trained_model_accuracy}")
            ...
            if os.path.exists(self.model_evaluation_config.gcp_local_path) == True:
                ...
                gcp_model = torch.load(
                    self.model_evaluation_config.gcp_local_path, map_location=torch.device('cpu'))
                logging.info("GCP model loaded")

                gcp_model_accuracy = self.evaluate(
                    model=gcp_model, df_test=df_test)
                logging.info(
                ...
                logging.info(
                    "GCP model is not available locally for comparison.")
