...
class ModelPredictor:
    ...
    def align_word_ids(self, texts: str, tokenizer: dict) -> list:
        logging.info(
            "Entered the align_word_ids method of Model predictor class")
        try:
            ...
            logging.info(
                "Exited the align_word_ids method of Model predictor class")
            return label_ids
    ...
    def evaluate_one_text(self, model: object, sentence: str, tokenizer: dict, ids_to_labels: dict) -> str:
        logging.info(
            "Entered the evaluate_one_text method of Model predictor class")
        try:
            ...
            logging.info(
                "Exited the evaluate_one_text method of Model predictor class")
    ...
    def initiate_model_predictor(self, sentence: str) -> str:
        ...
        try:
            os.makedirs(self.model_predictor_config.best_model_dir,
                        exist_ok=True)
            logging.info(
            ...
            logging.info(
                "Tokenizer Pickle file downloaded from google storage")
            ...
            logging.info(
                "ids to label Pickle file downloaded from google storage")
            ...
            model = torch.load(
                self.model_predictor_config.best_model_path, map_location=torch.device('cpu'))
